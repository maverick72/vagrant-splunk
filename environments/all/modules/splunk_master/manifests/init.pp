# splunk master manifest
class splunk_master ($rep_factor=3,$search_factor=2) {
  exec { 'enable-master':
    command => "/opt/splunk/bin/splunk edit cluster-config \
-mode master -replication_factor ${rep_factor} \
-search_factor ${search_factor} -auth admin:admin",
