# splunk peer node manifest
class splunk_peer_node {
  exec { 'enable-peer-node':
    command => "/opt/splunk/bin/splunk edit cluster-config -mode slave \
-master_uri https://${::master_ip}:8089 -replication_port 9887 -auth admin:admin",
